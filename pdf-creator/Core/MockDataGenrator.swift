...
struct MockDataGenrator {
    ...
    /// Legacy with mock data. Updated with new PDFItemProtocol usage
    func generateTableItemsForPDF() -> [PDFItemProtocol] {
        let headersCount = 5 /// as we use a special mock data, the number is static
        ...
        names.forEach { name in
            /// Need to keep the order, as this is a mock data with presentability value
            let data = ["\(index)", name, numbers[index], addresses[index], dateOfBirth[index]]
            
            let item = PDFItem(propertiesCount: headersCount, properties: data)
            result.append(item)
            index += 1
        }
